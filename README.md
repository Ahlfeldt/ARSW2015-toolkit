...
| `matlab/figs` | | Folder containing data figures generated by MATLAB code. |
| `matlab/section6` | | Folder containing code for the quantification and simulation of the model with exogenous fundamentals. |
| `matlab/section7` | | Folder containing code for the quantification and simulation of the model with endogenous agglomeration forces. |
...
| `matlab/prepdata_TD.m` | Loads various data files provided in the replication directory. Many files are not read in this teaching directory to save space. Output is saved as Matlab/data/prepdata_big_TD.mat. | You do not need to execute this file unless you want to add variables from the replication directory or you want to recreate the `prepdata_big_TD.mat` file from the original data in the replication directory. |
