...
%%% It holds HH constant and corresponds to the closed-city case
function [endog,ucprob,HH,converge,Ubar] = smodendog(param,fund,distvar,noj)
    ...
    % This program produces the following outputs
        ...
        % ucprob 
        % HH is total employment in the counterfactual (held constant)
        % converge indicator that solver has converged
        % Ubar is the expected utility in the counterfactual
...
HH=sum(HM);                                                                 % Total observed employment
...
EHM=Epp_j.*HH;                                                              % Compute workplace employment predicted commuting probabilities for given guesses of wages and floor space prices                    
                                                                            % Notice that we wont update HH, hence, we alway distribute the same total number of workers to residences and workplaces    
...
[x;maxLDwage;maxLDq;maxLDQ;maxLDtheta;MLSE]                                 % Output convergence measures to evaluate convergence in real time
